...
function ChatItem(props: { chat: Chat; selected?: boolean; onClick: () => void }) {
  ...
  return (
    <div className={`dark:text-gray-300 dark:hover:bg-gray-700 hover:bg-gray-100 dark:hover:text-white group flex items-center text-sm font-medium w-full space-x-2 justify-between overflow-hidden ${props.selected ? 'dark:bg-gray-700 bg-gray-100' : ''}`}>
      <button className="flex items-center justify-start space-x-2 min-w-0 w-full px-2 py-2 text-sm" onClick={props.onClick}>
        ...
        <div className="space-y-1 text-left w-full min-w-0">
          <div className="dark:text-gray-100 truncate w-full">
            {props.chat.title}
          </div>
      ...
      <div className="pr-2">
        <div className="flex items-center justify-center space-x-2">
          <button className="dark:text-gray-500 dark:hover:text-white transiton-all">
            <svg
              stroke="currentColor"
              fill="currentColor"
              strokeWidth="0"
              viewBox="0 0 1024 1024"
              className="w-6 h-6 sm:w-4 sm:h-4 text-gray-500"
              height="1em"
              width="1em"
              xmlns="http://www.w3.org/2000/svg"
            >
          ...
          <button className="dark:text-gray-500 dark:hover:text-white transiton-all" onClick={() => deleteChat()}>
            {
              deleting ? (
